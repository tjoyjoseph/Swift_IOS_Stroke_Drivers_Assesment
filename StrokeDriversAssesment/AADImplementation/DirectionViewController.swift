...
class DirectionViewController: UIViewController {
    ...
    @IBOutlet var tapImgSqCars: UITapGestureRecognizer!
    ...
    let MAXTIMEALLOWED = 900.0; ///CHANGEg
    var HELPUSED = 0;
    var score = 0;
    var time = 0.0;
    var timer = Timer();
...
//        }
    ...
    @IBAction func btnStart(_ sender: Any) {
        if gameinPlay == false{
            ...
            gameinPlay = true;
            timer = Timer.scheduledTimer(timeInterval: 0.2, target: self, selector: #selector(DirectionViewController.startTimer), userInfo: nil, repeats: true)
            DispatchQueue.global().async {
                if self.time >= self.MAXTIMEALLOWED {
                    self.showAlert(theTitle: "Time limit", theMessage: "Time limit reached")
                }
            }
...
//        }
    ...
    @IBAction func btnHelp(_ sender: UIButton) {
            ...
            HELPUSED = HELPUSED + 1;
            if HELPUSED < 3 {
            let path = Bundle.main.path(forResource: "Instructions", ofType: "txt", inDirectory: "DirectionResources")
            do {
    ...
    override func viewDidLoad() {
        ...
        var imageUrls = [URL]();
        imageUrls = Bundle.main.urls(forResourcesWithExtension: "png", subdirectory: "DirectionResources")!
...
//            for x in 0...3
//            {
//                imageNames.append(String(i)+String(x))
//            }
//        }
